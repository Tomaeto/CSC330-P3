      ...
      recursive integer*16 function collatz( i ) result (counter)
      ...
      !Postcondition: Returns length of Collatz sequence of value
        ...
        !If input value is not 1, perform Collatz formula and
        ! recursively increment counter 
        if (i == 1) then
                counter = 0
        else if (mod(i, 2) == 0) then
                counter = counter + collatz(i/2)
        else
                counter = counter + collatz(i * 3 + 1)
        endif
